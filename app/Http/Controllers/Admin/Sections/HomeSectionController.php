...
class HomeSectionController extends Controller
{
    public function index()
    {
        if (licenseType(2) && @settings('premium')->status) {
            ...
            $homeSections = HomeSection::all();
            #$homeSections = HomeSection::whereNot('alias', 'premium_items')->get(); //daha önce bu vardı
        }
    ...
    public function update(Request $request, HomeSection $homeSection)
    {
        ...
        if ($homeSection->alias == "premium_items") {
            $rules['json'] = ['nullable', 'json']; // Validate the JSON structure
        }
        ...
        if ($homeSection->alias == "premium_items") {
            $homeSection->json = json_decode($request->json, true); // Decode and save the JSON data
        }
