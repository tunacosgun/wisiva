    ...
    <div class="card">
        ...
        <div class="card-body my-1">
            @if ($translates->count() > 0)
                <form id="wisiva-submited-form" action="{{ route('admin.settings.language.translates.update') }}"
                    method="POST">
                    @csrf
                    @foreach ($translates as $translate)
                        <div class="wisiva-translate-box">
                            <div class="wisiva-translated-item d-block d-lg-flex bd-highlight align-items-center">
                                <div class="flex-grow-1 bd-highlight">
                                    <textarea class="wisiva-translate-key translate-fields form-control" rows="1" readonly>{{ $translate->key }}</textarea>
                                </div>
