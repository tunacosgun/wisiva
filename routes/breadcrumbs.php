...
Breadcrumbs::for('packages', function (BreadcrumbTrail $trail, $package) {
    $trail->parent('home');
    $trail->push($package->name, route('packages', $package->seo_url));
});

Breadcrumbs::for('offers', function (BreadcrumbTrail $trail, $offer) {
    $trail->parent('home');
    $trail->push($offer->title, route('offer', $offer->seo_url));
});
